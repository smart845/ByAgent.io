...
export aexport async function renderTopMoversHTML(){
  ...
  return `
    ...
    <div class="row" style="margin-top:12px;justify-content:center;">
      <button id="sendTestTelegramBtn" class="btn n-green" style="width:50%">📩 Отправить тестовый алерт в Telegram</button>
    </div>
  `;
  const btn = document.getElementById('sendTestTelegramBtn');
  if (btn) btn.addEventListener('click', sendTestTelegram);
    </div>
    `;
</div>
      </div>
